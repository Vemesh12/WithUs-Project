...
def create_password_reset_token(user: User) -> str:
    ...
    return create_access_token({"sub": user.email, "user_id": user.id, "purpose": "password_reset"}, expires)

def verify_password_reset_token(token: str, db: Session) -> Optional[User]:
    """Verify reset token and return the corresponding user if valid."""
    credentials_exception = HTTPException(
        status_code=status.HTTP_401_UNAUTHORIZED,
    ...
    try:
        ...
        if purpose != "password_reset" or user_id is None or email is None:
            raise credentials_exception
    except JWTError:
        raise credentials_exception
